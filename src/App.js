...
function App() {
  const [widgets, setWidgets] = useState([
    { id: new Date().getTime(), component: <ClockWidget />, area: 'mid1-widget', name: "Date and Time" },
    { id: new Date().getTime() + 2, component: <CalendarWidget />, area: 'mid2-widget', name: "Calendar" },
    ...
    <>
                ...
                <div style={{ padding: 10, }} className='widget-container' >
                  <div style={{ marginBottom: 10 }}>
                    ...
                    >
                      ...
                      <option value="mid1-widget">Middle 1</option>
                      <option value="mid2-widget">Middle 2</option>
      ...
      <div className="App">
        <div className="container">
          ...
          <div className='mid1-widget'>
            {widgets.length > 0 && widgets.map((widget, index) => {
              if (widget.area === 'mid1-widget') {
                return (
                  <div style={{ padding: 10, }} className='widget-container' >
                    <div style={{ marginBottom: 10 }}>
                      <select value={widget.area}
            ...
            {widgets.filter(widget => widget.area === 'mid1-widget').length < 2 &&
              <button
                className='add-widget-button'
                onClick={() => {
                  setShowWidgetModal(true)
                  setSelectedWidgetArea('mid1-widget')
                }}>+ Add Widget</button>
            }
          </div>
          <div className='mid2-widget'>
            {widgets.length > 0 && widgets.map((widget, index) => {
              if (widget.area === 'mid2-widget') {
                return (
                  <div style={{ padding: 10, }} className='widget-container'>
                    <div style={{ marginBottom: 10 }}>
                      <select value={widget.area}
            ...
            {widgets.filter(widget => widget.area === 'mid2-widget').length < 3 && widgets.filter(widget => widget.area === 'mid2-widget').length > 0 &&
              <button
                className='add-widget-button'
                onClick={() => {
                  setShowWidgetModal(true)
                  setSelectedWidgetArea('mid2-widget')
                }}>+ Add Widget</button>
            }
          </div>
